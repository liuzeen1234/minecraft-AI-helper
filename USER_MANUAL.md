...
AI 可放置、填充或清除方块，给予物品，生成实体，设置时间/天气，传送和生成/放置蓝图。单次填充或清除最多 10,000 方块、给予最多 64 件物品、生成最多 20 个实体。`execute_command` 不会自动执行任何命令——AI 只能把建议的命令文本预填到你的聊天输入框，需要你自己看清楚内容后手动按回车才会发送，真正执行时的权限完全由你自己当前的游戏权限决定；不要将其描述为可自动执行或提升权限执行原版命令。
